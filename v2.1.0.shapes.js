...
function redraw(){
        ...
        for(let item of state.strokes){
          if(item.view){
           ...
           if(item.type==="Shape"){
              rc.rectangle(item.points.x, item.points.y, item.points.width,item.points.height); // x, y, width, height
...
container.addEventListener("mousemove",(e)=>{
     ...
     // tell chatgpt about this 
        ...
        if(state.draw && state.SelectMode && state.currentEntite!=-1 && state.currentEntite!=undefined   && state.currentEntite !=null){
            ...
            const EntiteMove = state.strokes[state.currentEntite].points
            EntiteMove.x+=dx
            EntiteMove.y+=dy
...
container.addEventListener("mousedown",(e)=>{
      ...
      if(state.SelectMode  && state.draw){
        ...
        const IndexEntite =  HandelShapesDectection(CurrentX,CurrentY,state.strokes)
